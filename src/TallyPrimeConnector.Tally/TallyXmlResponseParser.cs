...
public sealed class TallyXmlResponseParser
{
    public IReadOnlyList<LedgerInfo> ParseLedgers(string xml)
    {
        ...
        return document.Descendants().Where(x => x.Name.LocalName.Equals("LEDGER", StringComparison.OrdinalIgnoreCase) && (Text(x, "NAME") is not null || Text(x, "MASTERID") is not null || Text(x, "GUID") is not null)).Select(x => new LedgerInfo(
    ...
    {
        var document = LoadSuccessfulEnvelope(xml);
        return document.Descendants().Where(x => x.Name.LocalName.Equals("GROUP", StringComparison.OrdinalIgnoreCase) && (Text(x, "NAME") is not null || Text(x, "MASTERID") is not null || Text(x, "GUID") is not null)).Select(x => new GroupInfo(
    ...
    private static VoucherInfo ParseVoucher(XElement voucher)
    {
        var entries = voucher.Elements().Where(x => x.Name.LocalName.Equals("LEDGERENTRIES.LIST", StringComparison.OrdinalIgnoreCase)).Select(entry =>
        {
            ...
            return new VoucherEntryInfo(Text(entry, "MASTERID") ?? Text(entry, "LEDGERNAME") ?? Guid.NewGuid().ToString("N"), Text(entry, "LEDGERNAME") ?? "Unknown Ledger", new TransactionAmount(Math.Abs(amount), direction), Text(entry, "BILLREF"));
        }).ToList();
