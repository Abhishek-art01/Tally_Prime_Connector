...
public sealed class DiscoveryTests
{
    ...
    [Fact] public async Task Mock_extraction_reports_voucher_sample() { var service = new ExtractionService(new VoucherService(new MockTallyVoucherProvider())); var company = new CompanyContext(new CompanyInfo("mock", "Demo", new DateTime(2026, 4, 1)), new ConnectionProfile("test", "localhost", 9000, TallyProtocol.HttpXml, TallyConnectionMethod.XmlHttp)); var result = await service.ExtractAsync(new(company, new("Sundry Debtors", ["Acme Trading"]), DateRange.Create(new DateOnly(2026, 4, 1), new DateOnly(2026, 4, 1))), null, CancellationToken.None); Assert.Single(result.Vouchers); Assert.Equal(2500.25m, result.Vouchers[0].Entries[0].Amount.Value); }
}
